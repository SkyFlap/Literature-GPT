...
def reshape_pdf_info(input_path: str, output_path: str) -> None:
    ...
    basename = os.path.basename(input_path)[0:-5]
    pdf_extract_db_path = os.path.join(output_path, f"{basename}_reshape.json")
...
def pdf_extract_kit(
    ...
    mfr_model, mfr_vis_processors = mfr_model_init(
        model_configs["model_args"]["mfr_weight"], device=device
    )
    ...
    if os.path.isdir(pdf):
        all_pdfs = [os.path.join(pdf, name) for name in os.listdir(pdf)]
    else:
        all_pdfs = [pdf]
    ...
    for idx, single_pdf in enumerate(all_pdfs):
        ...
        for idx, image in enumerate(img_list):
            ...
            mfd_res = mfd_model.predict(
                image, imgsz=img_size, conf=conf_thres, iou=iou_thres, verbose=True
            )[0]
        ...
        for idx, image in enumerate(img_list):
            ...
            for res in single_page_res:
                if int(res["category_id"]) in [
                    ...
                    6,
                    7,
                ]:  # categories that need to do ocr
                    xmin, ymin = int(res["poly"][0]), int(res["poly"][1])
                    xmax, ymax = int(res["poly"][4]), int(res["poly"][5])
        ...
        if vis:
            ...
            for idx, image in enumerate(img_list):
                single_page_res = doc_layout_result[idx]["layout_dets"]
                vis_img = (
                    Image.new("RGB", Image.fromarray(image).size, "white")
                    if render
                    else Image.fromarray(cv2.cvtColor(image, cv2.COLOR_RGB2BGR))
                )
                        ...
                        try:
                            if label in [13, 14]:  # render formula
                                ...
                                if True:  # render chinese
                                    ...
                                    window_img = tex2pil(
                                        [res["text"]], tex_type="text"
                                    )[0]
                            ratio = (
                                min(
                                    (x_max - x_min) / window_img.width,
                                    (y_max - y_min) / window_img.height,
                                )
                                - 0.05
                            )
                            window_img = window_img.resize(
                                (
                                    int(window_img.width * ratio),
                                    int(window_img.height * ratio),
                                )
                            )
                            vis_img.paste(
                                window_img,
                                (
                                    ...
                                    int(
                                        y_min + (y_max - y_min - window_img.height) / 2
                                    ),
                                ),
                            )
    ...
    return reshape_pdf_info(os.path.join(output_dir, f"{basename}.json"), output_dir)
