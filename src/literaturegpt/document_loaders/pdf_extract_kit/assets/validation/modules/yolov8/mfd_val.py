...
class MFDValidator(BaseValidator):
    ...
    def __call__(self, trainer=None, model=None, vis_box=True):
        ...
        for batch_i, batch in enumerate(bar):
            ...
            for im_path, ori_shape, re_shape, pred in zip(
                batch["im_file"], batch["ori_shape"], batch["resized_shape"], preds
            ):
                ...
                label_path = (
                    im_path.replace("/images/", "/labeling_anns/")[0:-4] + ".json"
                )
                if os.path.exists(label_path):
                    ...
                    with warnings.catch_warnings():
                        ...
                        label_path = (
                            im_path.replace("/images/", "/labels/")[0:-4] + ".txt"
                        )
                        gt_lines = np.genfromtxt(
                            label_path,
        ...
        coco_det_metric = COCODetection(
            dataset_meta=meta, metric=["bbox"], classwise=True
        )
        ...
        if text_info:
            ...
            if text_location == 0:
                cv2.rectangle(
                    img,
                    c1,
                    (c1[0] + int(t_size[0]), c1[1] + int(t_size[1] * 1.6)),
                    color,
                    -1,
                )
                ...
                cv2.rectangle(
                    img,
                    (c2[0] - int(t_size[0]), c1[1]),
                    (c2[0], c1[1] + int(t_size[1] * 1.6)),
                    color,
                    -1,
                )
                ...
                cv2.putText(
                    img,
                    text_info,
                    (c2[0] - int(t_size[0]), c1[1] + int(t_size[1] * 1.6)),
                    cv2.FONT_HERSHEY_TRIPLEX,
                    1,
                    [255, 255, 255],
                    1,
    ...
    def visulize(self, image, gt, pred, label_classes, thres=0.45, save_dir="vis"):
        ...
        if gt["bboxes"].shape[0] == 0 and pred["bboxes"].shape[0] == 0:
            return 0
        for idx, (bbox, score, label) in enumerate(
        ...
        for idx, (bbox, label, ignore) in enumerate(
            zip(gt["bboxes"], gt["labels"], gt["ignore_flags"])
        ):
            if ignore:
                color = (125, 125, 125)
            else:
                color = (0, 200, 0)
            img = self.plot_bbox(
                img, bbox, str(label), text_location=1, color=color, thickness=1
            )
