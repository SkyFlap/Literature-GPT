...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--weight", type=str, default="yolov8l.yaml", help="model.yaml path"
    )
    ...
    if args.weight.endswith(".pt"):  ## 评测单个模型，可视化box
        ...
        input_args1 = dict(
            model=args.weight, data=args.cfg1, imgsz=args.imsize, conf=0.25, iou=0.45
        )
        ...
        for model_name in natsorted(os.listdir(args.weight)):
            ...
            input_args1 = dict(
                model=model_path, data=args.cfg1, imgsz=args.imsize, conf=0.25, iou=0.45
            )
        ...
        with open(
            os.path.join(os.path.dirname(args.weight), "epoch_eval_results.json"),
            "w",
            encoding="utf-8",
        ) as f:
            f.write(json.dumps(epoch_eval_results, indent=2, ensure_ascii=False))
